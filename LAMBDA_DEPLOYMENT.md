...
```bash
# S3バケットを作成
aws s3 mb s3://gemini-news-analyzer --region ap-northeast-1
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:s3:::gemini-news-analyzer/config/*"
    },
    {
      "Effect": "Allow",
      "Action": [
      ...
      "Resource": "arn:aws:s3:::gemini-news-analyzer/responses/*"
    },
    {
      "Effect": "Allow",
      "Action": [
...
```bash
# 環境変数を設定
export LAMBDA_ROLE_ARN="arn:aws:iam::235217802903:role/lambda-gemini-news-analyzer"
export S3_BUCKET_NAME="gemini-news-analyzer"
...
GEMINI_API_KEYを環境変数として設定します。

```bash
# .envファイルからAPIキーを取得（ローカル環境）
source .env

# Lambda環境変数を設定
aws lambda update-function-configuration \
  --function-name gemini-news-analyzer \
  --environment "Variables={S3_BUCKET_NAME=gemini-news-analyzer,GEMINI_API_KEY=${GEMINI_API_KEY}}" \
  --region ap-northeast-1
```

**セキュリティ推奨**: AWS Secrets Managerを使用する場合:
...
# Lambda関数にSecrets Managerアクセス権限を追加
# (IAMロールに secretsmanager:GetSecretValue 権限を付与)

# lambda_handler.pyを修正してSecrets Managerから取得
```
...
# CloudWatch Logsを確認
aws logs tail /aws/lambda/gemini-news-analyzer --follow
```
...
# EventBridgeルールを作成
aws events put-rule \
  --name gemini-news-analyzer-daily \
...
aws events put-targets \
  --rule gemini-news-analyzer-daily \
...
aws lambda add-permission \
  --function-name gemini-news-analyzer \
  --statement-id gemini-news-analyzer-daily-event \
  --action 'lambda:InvokeFunction' \
  --principal events.amazonaws.com \
  --source-arn arn:aws:events:ap-northeast-1:YOUR_ACCOUNT_ID:rule/gemini-news-analyzer-daily \
  --region ap-northeast-1
```
...
# 今日の結果ファイルを確認
TODAY=$(date +%Y-%m-%d)
aws s3 ls s3://gemini-news-analyzer/responses/${TODAY}

# ファイルをダウンロードして確認
aws s3 cp s3://gemini-news-analyzer/responses/${TODAY}.txt ./
aws s3 cp s3://gemini-news-analyzer/responses/${TODAY}_articles.txt ./
...
# https://console.aws.amazon.com/cloudwatch/home?region=ap-northeast-1#logsV2:log-groups/log-group/$252Faws$252Flambda$252Fgemini-news-analyzer
```
...
aws lambda update-function-configuration \
  --function-name gemini-news-analyzer \
  --environment "Variables={S3_BUCKET_NAME=gemini-news-analyzer,GEMINI_API_KEY=YOUR_API_KEY}" \
  --region ap-northeast-1
```
...
```bash
# S3にアップロード
aws s3 cp lambda-layer.zip s3://gemini-news-analyzer/layers/lambda-layer.zip

# S3からLambda Layerを公開
aws lambda publish-layer-version \
  --layer-name gemini-news-analyzer-dependencies \
  --content S3Bucket=gemini-news-analyzer,S3Key=layers/lambda-layer.zip \
...
aws events disable-rule --name gemini-news-analyzer-daily --region ap-northeast-1
...
aws s3 sync s3://gemini-news-analyzer/responses/ ./responses/
